...
class PitchVisualizer:
    ...
    def __init__(self, config, pitch_config):
        ...
        # Voronoi settings
        self.show_voronoi = config['visualization'].get('show_voronoi', True)
        self.voronoi_alpha = config['visualization'].get('voronoi_alpha', 0.3)
    ...
    def render(self, person_detections, ball_detections, transformer, possession_info=None):
        ...
        if len(person_detections) > 0:
            ...
            if self.show_voronoi and len(pitch_positions) >= 3:
                # Separate by team
                team_0_mask = person_detections.class_id == 0
                team_1_mask = person_detections.class_id == 1

                if np.any(team_0_mask) and np.any(team_1_mask):
                    team_0_positions = pitch_positions[team_0_mask]
                    team_1_positions = pitch_positions[team_1_mask]
                    
                    # Get team colors
                    team_0_color = self.team_colors[0]
                    team_1_color = self.team_colors[1]
                    
                    # Draw Voronoi diagram for each team
                    pitch_view = draw_pitch_voronoi_diagram(
                        config=self.pitch_config,
                        xy=team_0_positions,
                        color=team_0_color,
                        alpha=self.voronoi_alpha,
                        pitch=pitch_view
                    )
